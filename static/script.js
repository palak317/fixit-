document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- INITIALIZATION ---
    injectAnimatedBackground(); // STEP 0: Inject 2036 Background
    createLoginModal(); // STEP 1: Show Login/Role Selection first
    fetchServices();    // Fetch data in background
    createChatModal();  // Initialize the Chat UI
    createBookingModal(); // Initialize Booking Details UI
    createItemModal();    // Initialize Marketplace Item Modal (Buy/Rent)
    injectSettingsUI();   // Initialize Settings & History UI (User Request)
    createAiDiagnosisUI(); // Initialize AI Diagnosis UI (User Request)
    ...
    function createLoginModal() {
        ...
        loginModal.innerHTML = `
            <div id="roleSelection" class="text-center space-y-8 animate-fade-up max-w-2xl px-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                    <!-- Customer Card -->
                    <button id="customerBtn" class="group relative p-8 bg-[#131b2e]/80 border border-white/10 rounded-2xl hover:border-blue-500/50 hover:bg-[#1a233b] transition-all duration-300 text-left hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(59,130,246,0.2)]">
                    ...
                    <!-- Provider Card -->
                    <button id="providerBtn" class="group relative p-8 bg-[#131b2e]/80 border border-white/10 rounded-2xl hover:border-purple-500/50 hover:bg-[#1a233b] transition-all duration-300 text-left hover:scale-[1.02] hover:shadow-[0_0_30px_rgba(168,85,247,0.2)]">
        ...
        // 3. Form Submit (Register & Show Dashboard)
        document.getElementById('regForm').onsubmit = (e) => {
            ...
            // Avatar logic (use uploaded or fallback)
            ...
            const newProvider = {
                ...
                image: avatarSrc,
                phone: phone,
                experience: experience, // Captured from new field
                isLocal: true
            };
            ...
            // RENDER PROVIDER DASHBOARD (User Request)
    ...
    function renderProviderDashboard(provider) {
        ...
        if(searchBar && searchBar.parentElement && searchBar.parentElement.parentElement) searchBar.parentElement.parentElement.classList.add('hidden');
        ...
        // 3. Inject "Explore Items" Button (User Request)
    ...
    function createSettingsModal() {
        ...
        // Mock History Data
        const historyData = [
            { name: "Raju Pipes Thane West", category: "Plumber", date: "Yesterday", price: 300, image: "https://api.dicebear.com/7.x/avataaars/svg?seed=Raju Pipes Thane West", id: "h1", location: "Thane West", rating: "4.8" },
            { name: "Current Shock Dadar", category: "Electrician", date: "12 Dec 2025", price: 500, image: "https://api.dicebear.com/7.x/avataaars/svg?seed=Current Shock Dadar", id: "h2", location: "Dadar", rating: "4.5" }
        ];

        const historyHTML = historyData.map((h, i) => `
           <div class="history-item flex items-center gap-4 p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-xl transition-all" data-id="${h.id}">
                <img src="${h.image}" class="w-12 h-12 rounded-lg bg-gray-800">
                <div class="flex-1">
                    <h4 class="font-bold text-white">${h.name}</h4>
                    <p class="text-sm text-gray-400">${h.category} • ${h.date}</p>
                </div>
                <div class="flex gap-2">
                     <button class="btn-rate-service p-2 bg-yellow-600/20 text-yellow-400 rounded-full hover:bg-yellow-600 hover:text-white transition-all" title="Rate Service" onclick="openRatingModal('${h.name}')">
                        <i data-lucide="star" class="w-4 h-4"></i>
                    </button>
                    <button class="btn-contact-history p-2 bg-blue-600/20 text-blue-400 rounded-full hover:bg-blue-600 hover:text-white transition-all" title="Call Again" onclick="openChat({name: '${h.name}', image: '${h.image}', isBusy: false})">
                ...
                <div class="overflow-y-auto p-6 space-y-8 flex-1 scrollbar-hide">
                    ...
                    <div>
                        <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                            <i data-lucide="clock" class="text-emerald-400 w-5 h-5"></i> Previous Services
                        </h3>
                        <div class="space-y-3">
                            ${historyHTML}
                        </div>
        ...
        window.openRatingModal = createRatingModal;
    }
    ...
    function openDetails(service) {
        // Removed blocking check for busy status
        ...
        // Inject Name above Location row (User Request)
        ...
        // --- NEW: Inject Chat Button (Replaces Call Now) ---
        const btnContainer = detailsModal.querySelector('.mt-8.flex.gap-3');
        if(btnContainer) {
            // Logic to handle Busy State in buttons
            ...
            // Attach listener to new Chat button
            document.getElementById('startChatBtn').onclick = () => openChat(service);
        }
